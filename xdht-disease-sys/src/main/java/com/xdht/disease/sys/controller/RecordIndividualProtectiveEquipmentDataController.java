...
public class RecordIndividualProtectiveEquipmentDataController {
    ...
    public  ResponseEntity<Result<List<RecordIndividualProtectiveEquipmentData>>> recordList(@CurrentUser User user, @RequestBody RecordIndividualProtectiveEquipmentDataRequest recordDataRequest) {
        return new ResponseEntity<>(Result.ok(recordDataService.queryList(recordDataRequest)), HttpStatus.OK);
    }
    @RequestMapping(value = "/recordPage", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation(value = "分页查询")
    public ResponseEntity<Result<PageResult<RecordIndividualProtectiveEquipmentData>>> recordPage(@CurrentUser User user, @RequestBody RecordIndividualProtectiveEquipmentDataRequest recordDataRequest, @RequestParam Integer pageNum, @RequestParam Integer pageSize) {
    ...
    public ResponseEntity<Result<RecordIndividualProtectiveEquipmentData>> add(@CurrentUser User user, @RequestBody RecordIndividualProtectiveEquipmentData recordData) {
        return new ResponseEntity<>(Result.ok(recordDataService.add(recordData)), HttpStatus.OK);
    }

    @RequestMapping(value = "/delete", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation(value = "删除")
    public ResponseEntity<Result<RecordIndividualProtectiveEquipmentData>> delete(@CurrentUser User user, @RequestParam Long id) {
        return new ResponseEntity<>(Result.ok(recordDataService.delete(id)), HttpStatus.OK);
    }

    @RequestMapping(value = "/update", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation(value = "修改")
    public ResponseEntity<Result<RecordIndividualProtectiveEquipmentData>> update(@CurrentUser User user, @RequestBody RecordIndividualProtectiveEquipmentData recordData) {
